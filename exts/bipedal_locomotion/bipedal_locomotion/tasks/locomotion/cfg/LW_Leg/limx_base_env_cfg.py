...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityCommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-1.0, 1.0), lin_vel_y=(-0.0, 0.0), ang_vel_z=(-1, 1), heading=(-math.pi, math.pi)
        ),
    )
...
class ObservarionsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        joint_vel = ObsTerm(func=mdp.joint_vel_rel, noise=GaussianNoise(mean=0.0, std=0.1), 
                            params={"asset_cfg": SceneEntityCfg("robot", joint_names="", preserve_order=True)},
...
class EventsCfg:
    ...
    add_link_mass = EventTerm(
        ...
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="[rightleft]_.*"),
    ...
    robot_physics_material = EventTerm(
        ...
        params={
            ...
            "static_friction_range": (0.3, 1.2), # (0.4, 1.2)
            "dynamic_friction_range": (0.3, 1.0), # (0.7, 0.9)
            "restitution_range": (0.0, 0.5), # (0.0, 1.0)
            "num_buckets": 64,
        },
    )
...
class RewardsCfg:
    ...
    rew_keep_ankle_pitch_zero_in_air = RewTerm(
        func=mdp.keep_ankle_pitch_zero_in_air, weight=0.5, params={"asset_cfg": SceneEntityCfg("robot"),
                                                                   "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["[rightleft]_foot_link"]),}
    )

    # penalizations
    pen_base_height = RewTerm(func=mdp.base_com_height, params={"target_height": 0.69}, weight=-50.0)
    pen_lin_vel_z = RewTerm(func=mdp.lin_vel_z_l2, weight=-10) # -0.5
    pen_ang_vel_xy = RewTerm(func=mdp.ang_vel_xy_l2, weight=-0.05)

    pen_joint_torque = RewTerm(func=mdp.joint_torques_l2, weight=-8e-5, params={"asset_cfg": SceneEntityCfg("robot", joint_names="")})
    pen_joint_torque_wheel = RewTerm(func=mdp.joint_torques_l2, weight=-1.6e-4, params={"asset_cfg": SceneEntityCfg("robot", joint_names="")})
    pen_joint_accel = RewTerm(func=mdp.joint_acc_l2, weight=-1e-6, params={"asset_cfg": SceneEntityCfg("robot", joint_names="")}) # -2.5e-7
    pen_joint_accel_wheel = RewTerm(func=mdp.joint_acc_l2, weight=-1.5e-7, params={"asset_cfg": SceneEntityCfg("robot", joint_names="")})

    pen_action_rate = RewTerm(func=mdp.action_rate_l2, weight=-0.5) # -0.01)
    pen_joint_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-2.0, params={"asset_cfg": SceneEntityCfg("robot", joint_names="")})
    ...
    pen_feet_distance = RewTerm(
        ...
        weight=-100,
        params={"min_feet_distance": 0.115,"feet_links_name": ["[rightleft]_foot_link"]}
    )
    pen_feet_regulation = RewTerm(
        func=mdp.feet_regulation,
        weight=-0.2, # -0.05,
        params={"asset_cfg": SceneEntityCfg("robot", body_names=["[rightleft]_foot_link"]),
                "base_height_target": 0.69, "foot_radius": 0.071},
    )
    ...
    pen_joint_vel_wheel_l2 = RewTerm(func=mdp.joint_vel_l2, weight=-5.0e-03
                               params={"asset_cfg": SceneEntityCfg("robot", joint_names="")})
...
@configclass
class LWLegEnvCfg(ManagerBasedRLEnvCfg):
